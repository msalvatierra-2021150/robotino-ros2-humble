          ...
          std::shared_ptr<frontier_interfaces::srv::FrontierGoal::Response> response)
{
    ...
    if (map.data.empty() || map.info.width == 0 || map.info.height == 0) {
        RCLCPP_WARN(this->get_logger(), "No valid map received yet.");
        return;
    }
    ...
    if (
        ...
        invalid_goal.header.stamp = this->get_clock()->now();
    ...
    if (frontierRegions_.empty()) {
        RCLCPP_WARN(this->get_logger(), "No frontier regions found. Cannot select goal.");
        return;
    }

    if (request->goal_rank < 0 ||
        request->goal_rank >= static_cast<int>(frontierRegions_.size())) {
        RCLCPP_WARN(
            this->get_logger(),
